...
export function Hourly() {
    ...
    useEffect(() => {
        if (fetchDatas) {
            for (const item of fetchDatas.list.slice(0,10)) {
                setItems(prev => [
                    ...prev,
                    {
                        dt: item.dt,
                        ...
                        humidity: Number(item.main.humidity.toFixed(0)),
    ...
    return (
        <div>
            <section className="bg-white dark:bg-gray-900 dark:bg-opacity-80 bg-opacity-75 rounded-lg shadow p-5">
                <h2 className="font-bold dark:text-gray-50 text-xl mb-4">Prochaines heures</h2>
                <div>
                            ...
                            <div key={item.dt}>
                                <div className="flex content-center justify-center">
                                    <p className="text-xs text-black">
                                        { new Date(item.dt * 1000).toLocaleTimeString('fr', { hour: '2-digit'}) }
                                    </p>
                                </div>
